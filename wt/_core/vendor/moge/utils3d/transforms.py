...
def intrinsics_from_fov_xy(
    fov_x: float | torch.Tensor, fov_y: float | torch.Tensor
) -> torch.Tensor:
    """
...
def perspective_to_intrinsics(perspective: torch.Tensor) -> torch.Tensor:
    ...
    ret = (
        ...
        @ torch.diag(
            torch.tensor(
                [1, -1, -1], dtype=perspective.dtype, device=perspective.device
            )
        )
    )
...
def extrinsics_to_view(extrinsics: torch.Tensor) -> torch.Tensor:
    ...
    return (
        extrinsics
        * torch.tensor(
            [1, -1, -1, 1], dtype=extrinsics.dtype, device=extrinsics.device
        )[:, None]
    )
...
def view_to_extrinsics(view: torch.Tensor) -> torch.Tensor:
    ...
    return (
        view
        * torch.tensor([1, -1, -1, 1], dtype=view.dtype, device=view.device)[:, None]
    )
...
def matrix_to_euler_angles(matrix: torch.Tensor, convention: str) -> torch.Tensor:
    ...
    """
    if not all(c in "XYZ" for c in convention) or not all(
        c in convention for c in "XYZ"
    ):
        raise ValueError(f"Invalid convention {convention}.")
    ...
    if tait_bryan:
        central_angle = torch.asin(
            matrix[..., i2, i0] * (-1.0 if i2 - i0 in [-1, 2] else 1.0)
        )
    else:
...
def axis_angle_to_quaternion(
    axis_angle: torch.Tensor, eps: float = 1e-12
) -> torch.Tensor:
...
def quaternion_to_matrix(quaternion: torch.Tensor, eps: float = 1e-12) -> torch.Tensor:
    ...
    B = torch.stack([zeros, -z, y, z, zeros, -x, -y, x, zeros], dim=-1).unflatten(
        -1, (3, 3)
    )
...
def interpolate_extrinsics(
    ...
    return torch.inverse(
        interpolate_transform(torch.inverse(ext1), torch.inverse(ext2), t)
    )


def interpolate_view(
    view1: torch.Tensor, view2: torch.Tensor, t: Union[Number, torch.Tensor]
):
...
def interpolate_transform(
    ...
    pos = (1 - t[..., None]) * transform1[..., :3, 3] + t[..., None] * transform2[
        ..., :3, 3
    ]
...
def to4x4(R: torch.Tensor, t: torch.Tensor):
    ...
    return torch.cat(
        [
            ...
            torch.tensor([0, 0, 0, 1], dtype=R.dtype, device=R.device).expand(
                *R.shape[:-2], 1, 4
            ),
        ],
        dim=-2,
    ...
    return torch.cat(
        [
            torch.cat(
                [
                    torch.eye(
                        2, dtype=translation.dtype, device=translation.device
                    ).expand(*translation.shape[:-1], -1, -1),
                    translation[..., None],
                ],
                dim=-1,
            ),
            torch.tensor(
                [[0, 0, 1]], dtype=translation.dtype, device=translation.device
            ).expand(*translation.shape[:-1], -1, -1),
        ],
        dim=-2,
    )
...
def scale_2d(scale: Union[float, torch.Tensor], center: torch.Tensor = None):
    ...
    if center is None:
        center = torch.zeros(2, dtype=scale.dtype, device=scale.device).expand(
            *scale.shape, -1
        )
    return torch.cat(
        [
