    ...
    if vertices is not None:
        ...
        if backslash is None:
            ...
            backslash = torch.norm(
                ...
                vertices[faces_idx[..., 1]] - vertices[faces_idx[..., 3]], p=2, dim=-1
            )
    if backslash is None:
...
def compute_vertex_normal_weighted(
    ...
    face_normal = (
        face_normal[:, :, None, :].expand(-1, -1, 3, -1) * face_angle[..., None]
    )
    vertex_normal = torch.index_put(
        torch.zeros_like(vertices),
...
def subdivide_mesh_simple(
    ...
    for _ in range(n):
        ...
        faces = torch.cat(
            [
                torch.stack(
                    [faces[:, 0], n_vertices + uni_inv[0], n_vertices + uni_inv[2]],
                    axis=1,
                ),
                torch.stack(
                    [faces[:, 1], n_vertices + uni_inv[1], n_vertices + uni_inv[0]],
                    axis=1,
                ),
                torch.stack(
                    [faces[:, 2], n_vertices + uni_inv[2], n_vertices + uni_inv[1]],
                    axis=1,
                ),
                torch.stack(
                    [
                        n_vertices + uni_inv[0],
                        n_vertices + uni_inv[1],
                        n_vertices + uni_inv[2],
                    ],
                    axis=1,
                ),
            ],
...
def laplacian(
    vertices: torch.Tensor, faces: torch.Tensor, weight: str = "uniform"
) -> torch.Tensor:
    ...
    if weight == "cotangent":
        for i in range(3):
            ...
            sum_weights = torch.index_add(
                sum_weights, -1, faces[:, (i + 1) % 3], cot_angle
            )
            sum_verts = torch.index_add(
                sum_verts,
                -2,
            ...
            sum_weights = torch.index_add(
                sum_weights, -1, faces[:, (i + 2) % 3], cot_angle
            )
    elif weight == "uniform":
        for i in range(3):
            sum_verts = torch.index_add(
                sum_verts, -2, faces[:, i], face_verts[..., (i + 1) % 3, :]
            )
            sum_weights = torch.index_add(
