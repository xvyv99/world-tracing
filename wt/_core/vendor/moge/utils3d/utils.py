...
def image_pixel_center(
    ...
    if bottom is None:
        bottom = height
    u = torch.linspace(
        left + 0.5, right - 0.5, right - left, dtype=dtype, device=device
    )
    v = torch.linspace(
        top + 0.5, bottom - 0.5, bottom - top, dtype=dtype, device=device
    )
    u, v = torch.meshgrid(u, v, indexing="xy")
...
def image_mesh(
    ...
    if mask is not None:
        ...
        faces, uv, indices = mesh.remove_unreferenced_vertices(
            faces, uv, return_indices=True
        )
...
def depth_aliasing(
    ...
    if mask is None:
        diff_max = (
            F.max_pool2d(depth, kernel_size, stride=1, padding=kernel_size // 2) - depth
        )
        diff_min = (
            F.max_pool2d(-depth, kernel_size, stride=1, padding=kernel_size // 2)
            + depth
...
def point_to_normal(point: torch.Tensor, mask: torch.Tensor = None) -> torch.Tensor:
    ...
    pts = F.pad(
        point.permute(0, 3, 1, 2), (1, 1, 1, 1), mode="constant", value=1
    ).permute(0, 2, 3, 1)
...
def masked_max(
    ...
    if dim is None:
        return torch.where(
            mask,
            input,
            torch.tensor(-torch.inf, dtype=input.dtype, device=input.device),
        ).max()
    else:
        return torch.where(
            mask,
