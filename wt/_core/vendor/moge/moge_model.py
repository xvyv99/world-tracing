...
from wt._core.vendor.moge.utils.geometry_torch import image_plane_uv, point_map_to_depth
from wt._core.vendor.moge.utils.wrap_module import (
    wrap_dinov2_attention_with_fa3,
...
MODEL_ZOO_DIR = None
"""Default download location for pre-trained MoGe checkpoints.

The published ``wt`` inference release does not exercise this code path —
encoder weights are restored from the released ``wt`` checkpoints directly.
Set this to a local directory (or override ``MoGeModel.from_pretrained(...,
cache_path=...)``) only if you intend to fine-tune from the original MoGe
weights.  The official MoGe weights are published at
...
class MoGeModel(nn.Module):
    ...
    def __init__(
        ...
        **deprecated_kwargs,
        ...
        # Minimal modifications have been made: removing irrelevant code, unnecessary
        # warnings and fixing importing issues.
        from wt._core.vendor.moge.dinov2 import hub

        hub_loader = getattr(hub, encoder)
        ...
        self.head = Head(
            ...
            if output_mask and not split_head
            else [3, 1],
            dim_proj=dim_proj,
    ...
    def from_pretrained(
        ...
        Either arch must be specified to use a predefined model architecture, or
        both `model_config` and `model_weights` must be
        provided to load a custom model.

        Args:
            model_name: Predefined model name. Currently only "vitl" is supported.
            ...
            model_kwargs: Additional keyword arguments to override the parameters
                in the `model_config`.
            cache_path: Optional path to cached model weights. If provided, will
                attempt to load from this path first, falling back to default
                paths on failure.
        ...
        Raises:
            ValueError: If arch is not "vitl", or if neither arch nor both
        ...
        if model_name is not None:
            if model_name != "vitl":
                raise ValueError(
                    # Raising ValueError to allow supporting model models in future.
        ...
        if cache_path is not None:
            ...
            cache_config_path = cache_dir / f"moge-{model_name}-config.json"
            cache_weights_path = cache_dir / f"moge-{model_name}.safetensors"
            logger.info(
                "load_models_code_path: Attempting to load MoGe from cache",
                cache_path=cache_path,
                cache_config_path=str(cache_config_path),
                cache_weights_path=str(cache_weights_path),
            )
            try:
                with cache_config_path.open("r") as f:
                    model_config_dict = json.load(f)
                    if model_kwargs is not None:
                        model_config_dict.update(model_kwargs)
                with safetensors.safe_open(
                    cache_weights_path, framework="pt"
                ) as weights:
                    state_dict = {
                        k: weights.get_tensor(k)
                        for k in weights.keys()  # noqa: SIM118
                    }
                model = cls(
                    **model_config_dict,
                ...
                logger.info(
                    "load_models_code_path: Loaded MoGe from cache",
                    cache_path=cache_path,
                )
                return model
            except Exception as e:  # pylint: disable=broad-exception-caught
                logger.error(
                    "load_models_code_path: Failed to load MoGe from cache. "
        ...
        model.load_state_dict(state_dict)
        logger.info(
            "load_models_code_path: Loaded MoGe from default path",
            weights_path=str(model_weights),
        )
        return model
    ...
    def enable_fa3(self):
        for i in range(len(self.backbone.blocks)):
            self.backbone.blocks[i].attn = wrap_dinov2_attention_with_fa3(
                self.backbone.blocks[i].attn
            )
    ...
    def infer(
        ...
        image = image.to(device=self.device, dtype=self.weights_dtype)
        ...
        if expected_area != area:
            expected_width, expected_height = (
                int(original_width * (expected_area / area) ** 0.5),
        ...
        # Perform point operations in high precision.
        ...
        depth, fov_x, fov_y, z_shift = point_map_to_depth(
            points,
        ...
        intrinsics = utils3d.intrinsics_from_fov_xy(fov_x, fov_y)
        ...
        if force_projection:
            ...
            points = (
                points
                + torch.stack(
                    [torch.zeros_like(z_shift), torch.zeros_like(z_shift), z_shift],
                    dim=-1,
                )[..., None, None, :]
            )
