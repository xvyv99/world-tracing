...
from wt._core.vendor.moge.utils import geometry_numpy


def weighted_mean(
    x: torch.Tensor,
    w: torch.Tensor = None,
    dim: int | torch.Size = None,
    keepdim: bool = False,
    eps: float = 1e-7,
) -> torch.Tensor:
    if w is None:
...
def point_map_to_depth(
    ...
    downsample_size: tuple[int, int] | None = (64, 64),
    use_torch_solver: bool = False,
):
    """
    ...
    - `downsample_size: Tuple[int, int] | None` in (height, width), the size of
      the downsampled map. Downsampling produces approximate solution and is
      efficient for large maps. If `None`, use the original resolution.
    ...
    if use_torch_solver:
        optim_shift, optim_focal = _solve_shift_focal_torch(points_lr, uv_lr, mask_lr)
    else:
        ...
        for i in range(points.shape[0]):
            ...
            optim_shift_i, optim_focal_i = geometry_numpy.solve_optimal_shift_focal(
...
def recover_focal_shift(
    ...
    if use_torch_solver:
        optim_shift, optim_focal = _solve_shift_focal_torch(
            points_lr, uv_lr, mask_lr, focal=focal
        )
        optim_shift = optim_shift.reshape(shape[:-3])
        ...
        for i in range(points.shape[0]):
            ...
            if uv_lr_i_np.shape[0] < 2:
                ...
                continue
            if focal is None:
                optim_shift_i, optim_focal_i = geometry_numpy.solve_optimal_focal_shift(
                    uv_lr_i_np, points_lr_i_np
                )
                optim_focal.append(float(optim_focal_i))
            else:
                optim_shift_i = geometry_numpy.solve_optimal_shift(
...
def _solve_shift_focal_torch(
    points_lr: torch.Tensor,
    uv_lr: torch.Tensor,
    mask_lr: torch.Tensor | None,
    focal: torch.Tensor | None = None,
) -> tuple[torch.Tensor, torch.Tensor]:
    """GPU-friendly least-squares focal/shift estimate on downsampled maps."""
    batch_size = points_lr.shape[0]
    dtype = points_lr.dtype
    device = points_lr.device

    optim_shift = torch.zeros(batch_size, device=device, dtype=dtype)
    optim_focal = torch.ones(batch_size, device=device, dtype=dtype)
    points_flat = points_lr.reshape(batch_size, -1, 3)
    uv_flat = uv_lr.reshape(1, -1, 2).expand(batch_size, -1, -1)
    if mask_lr is None:
        weights = torch.ones(
            (batch_size, points_flat.shape[1]), device=device, dtype=dtype
        )
    else:
        weights = mask_lr.reshape(batch_size, -1).to(dtype=dtype)

    valid_point_count = weights.sum(dim=1)
    ...
    if focal is None:
        # Weighted linear least squares for [focal, shift], solved in closed form
        # without scalar extraction from GPU tensors.
        s11 = (weights * (x * x + y * y)).sum(dim=1)
        s12 = (weights * (-x * u - y * v)).sum(dim=1)
        s22 = (weights * (u * u + v * v)).sum(dim=1)
        t1 = (weights * (x * u * z + y * v * z)).sum(dim=1)
        t2 = (weights * (-(u * u + v * v) * z)).sum(dim=1)

        det = s11 * s22 - s12 * s12
        det_safe = torch.where(
            det.abs() < 1e-8,
            torch.where(
                det >= 0, torch.full_like(det, 1e-8), torch.full_like(det, -1e-8)
            ),
            det,
        )

        focal_est = (t1 * s22 - t2 * s12) / det_safe
        shift_est = (s11 * t2 - s12 * t1) / det_safe
        valid = valid_point_count >= 2
        optim_focal = torch.where(valid, focal_est, optim_focal)
        optim_shift = torch.where(valid, shift_est, optim_shift)
    else:
        optim_focal = focal.to(device=device, dtype=dtype).clone()
        rhs_x = u * z - x * optim_focal[:, None]
        rhs_y = v * z - y * optim_focal[:, None]
        numerator = (weights * (-u * rhs_x - v * rhs_y)).sum(dim=1)
        denominator = (weights * (u * u + v * v)).sum(dim=1).clamp_min(1e-8)
        shift_est = numerator / denominator
        if mask_lr is None:
            optim_shift = shift_est
        else:
            valid = valid_point_count >= 2
            optim_shift = torch.where(valid, shift_est, optim_shift)
