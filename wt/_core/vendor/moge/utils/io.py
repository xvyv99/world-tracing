...
def write_rgbd_zip(
    ...
    depth: np.ndarray | bytes,
    mask: np.ndarray | bytes,
    segmentation_mask: np.ndarray | bytes = None,
    segmentation_labels: dict[str, int] | bytes = None,
    intrinsics: np.ndarray = None,
    normal: np.ndarray = None,
    normal_mask: np.ndarray = None,
    meta: dict[str, Any] | bytes = None,
    *,
    ...
    if isinstance(image, bytes):
        ...
        image_bytes = cv2.imencode(
            ".jpg", image, [cv2.IMWRITE_JPEG_QUALITY, image_quality]
        )[1].tobytes()
    ...
    if isinstance(depth, bytes):
        ...
        if depth_type == "linear":
            ...
            depth = (
                (np.log(depth.clip(near, far) / near) / np.log(far / near)).clip(0, 1)
                * 65535
            ).astype(np.uint16)
            depth_bytes = cv2.imencode(
                ".png", depth, [cv2.IMWRITE_PNG_COMPRESSION, png_compression]
    ...
    if segmentation_mask is not None:
        if isinstance(segmentation_mask, bytes):
            ...
            segmentation_mask_bytes = cv2.imencode(".png", segmentation_mask)[
                1
            ].tobytes()
        assert (
    ...
    if normal is not None:
        if isinstance(normal, bytes):
            ...
            normal = ((normal * [0.5, -0.5, -0.5] + 0.5).clip(0, 1) * 65535).astype(
                np.uint16
            )
        ...
        normal_mask_bytes = cv2.imencode(".png", normal_mask.astype(np.uint8) * 255)[
            1
        ].tobytes()
...
def read_rgbd_zip(
    file: str | Path | IO, return_bytes: bool = False
) -> dict[str, np.ndarray | dict[str, Any] | bytes]:
    """
    ...
    with zipfile.ZipFile(file, "r") as z:
        ...
        if not return_bytes:
            image = cv2.imdecode(
                np.frombuffer(z.read("image.jpg"), np.uint8), cv2.IMREAD_COLOR
            )
        ...
        if not return_bytes:
            depth = cv2.imdecode(
                np.frombuffer(z.read(depth_name), np.uint8), cv2.IMREAD_UNCHANGED
            )
        ...
        if "mask.png" in z.namelist():
            ...
            if not return_bytes:
                mask = (
                    cv2.imdecode(
                        np.frombuffer(z.read("mask.png"), np.uint8),
        ...
        if "normal.png" in z.namelist():
            ...
            if "normal_mask.png" in z.namelist():
                ...
                normal_mask = (
                    cv2.imdecode(
                        np.frombuffer(normal_mask, np.uint8), cv2.IMREAD_UNCHANGED
                    )
                    > 0
                )
            else:
...
def read_rgbxyz(
    ...
    if is_archive:
        ...
        file.mkdir(parents=True, exist_ok=True)
        image = cv2.cvtColor(
            cv2.imread(str(file / "image.jpg"), cv2.IMREAD_COLOR), cv2.COLOR_BGR2RGB
        )
        points = cv2.cvtColor(
            cv2.imread(str(file / "points.exr"), cv2.IMREAD_UNCHANGED),
            cv2.COLOR_BGR2RGB,
        )
